      ...
      <TabContext value={value}>
        <Grid container spacing={2} direction="row" justifyContent="center">
          <Grid
            ...
            md={10}
            lg={1}
            sx={{ textAlign: "right", marginTop: { xs: "0px", lg: "3px" } }}
          >
            <NftFilter />
          </Grid>
        ...
        <TabPanel value="2">
          <Grid container spacing={2}>
            {dataContext?.nftItems
              .filter((item) => item.status === "trending")
              .map((nft) => (
                <Grid
        ...
        <TabPanel value="4">
          <Grid container spacing={2}>
            {dataContext?.nftItems
              .filter((item) => item.category === "animated")
              .map((nft) => (
                <Grid
                  key={"nft-animated-" + nft.key}
                  item
                  xs={12}
                  md={4}
                  xl={3}
        ...
        <TabPanel value="5">
          <Grid container spacing={2}>
            {dataContext?.nftItems
              .filter((item) => item.category === "polymorphous")
              .map((nft) => (
                <Grid
                  key={"nft-polymorphous-" + nft.key}
                  item
                  xs={12}
                  md={4}
                  xl={3}
