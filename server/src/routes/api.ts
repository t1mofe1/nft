...
router.use('/', async (req, res) => {
	...
	if (result.type === ResponseType.RESPONSE) {
		// We set the provided status and headers and just the send the payload back to the client
		result.headers.forEach(({ name, value }) => res.setHeader(name, value));

		res.status(result.status).json(result.payload);
	} else if (result.type === ResponseType.MULTIPART_RESPONSE) {
		...
		await result.subscribe(result => {
			...
			const data = ['', 'Content-Type: application/json; charset=utf-8', `Content-Length: ${chunk.length}`, '', chunk];
		...
		res.write('\r\n-----\r\n');
		res.end();
	} else if (result.type === ResponseType.PUSH) {
		// Indicate we're sending an event stream to the client
		res.writeHead(200, {
			Connection: 'keep-alive',
